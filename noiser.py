...
def read_random_batch(all_files, 
                      resample, 
                      batch_size, 
                      workers, 
                      new_length):
...
def make_noisy_file(clean_file_path, cfg, noise_files, resample):
    ...
    file_name = os.path.join(cfg.result_dir, 
                             clean_file_path.split('/')[-1].split('.')[0])
    ...
    noises_batch = read_random_batch(noise_files, 
                                     resample,
    ...
    with get_context("spawn").Pool(cfg.workers) as p:
            files = p.starmap(save_file, [(noised[i], i) for i in range(cfg.num_copies)])
    output_file.close()
...
if __name__ == "__main__":
    ...
    resample  = ta.transforms.Resample(orig_freq= cfg.noise_sr, 
                                       new_freq = cfg.source_sr, 
                                       resampling_method = 'sinc_interpolation')
    
        
    for f in tqdm(clean_files):
        make_noisy_file(f, cfg, noise_files, resample)
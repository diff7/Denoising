...
class BaseTrainer:
    ...
    def _resume_checkpoint(self):
        ...
        print(
            f"Model checkpoint loaded. Training will begin in {self.start_epoch} epoch."
        )
    ...
    def _save_checkpoint(self, epoch, is_best=False):
        ...
        if is_best:
            ...
            torch.save(
                state_dict, (self.checkpoints_dir / "best_current_model.tar").as_posix()
            )
    ...
    def _print_networks(nets: list):
        print(
            f"This project contains {len(nets)} networks, the number of the parameters: "
        )
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.validation_interval != 0 and epoch % self.validation_interval == 0:
                print(
                    f"[{timer.duration()} seconds] Training is over. Validation is in progress..."
                )
